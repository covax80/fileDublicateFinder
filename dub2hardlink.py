...
#----------------------------------------------------------------------
# Description: Util to clean up network shares - it can remove dublicated files and replace them with hardlinks.
# Author:  Artyom Breus <Artyom.Breus@gmail.com>
# Created at: Sep 7 17:02:07 VLAT 2020
# Computer: COVAX-WS777#
# Copyright (c) 2020-2021 Artyom Breus  All rights reserved.
#
#----------------------------------------------------------------------
...
def cur_execute(cursor, query, data):		
...
def make_hardlinks():
	global CONN, DUBLICATE_FILENAMES
	cursor  = CONN.cursor()	
	query = """SELECT filename, folder, filehash FROM files ORDER BY filename,folder ASC"""
	cursor.execute(query)
	full_filename = None
	#same_files = {}
	old_hash = None
	file_for_link = None
	for filename, folder, filehash in cursor.fetchall():
		full_filename = (os.path.join( folder, filename ))
		if filehash != old_hash:
		    old_hash = filehash
		    file_for_link = full_filename
		    print("\n\n")
		    continue
		os.remove(full_filename)
		os.link(file_for_link, full_filename)
		print("hardlink {} {}".format(file_for_link, full_filename))
		#same_files[filehash] = (same_files.get(filehash,[])).append(full_filename)	
...
if __name__ == '__main__':
	if len(sys.argv) > 1:
		...
		main( folders )
        	...
        	print('Usage: python dub2hardlink.py folder1 folder2 folder3 ...')
